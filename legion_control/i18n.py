...
_ENTRIES: Final = (
    ...
    ("Efectos animados", "Animated effects", "Effets animés", "动态效果", "Анимированные эффекты"),
    (
        "Los anima el equipo enviando frames verificados; no usa efectos del firmware",
        "The computer animates them by sending verified frames; no firmware effect is used",
        "L’ordinateur les anime en envoyant des trames vérifiées ; aucun effet du micrologiciel",
        "由本机发送经验证的帧来播放动画，不使用固件效果",
        "Их анимирует компьютер, отправляя проверенные кадры; эффекты прошивки не используются",
    ),
    ("Efecto", "Effect", "Effet", "效果", "Эффект"),
    (
        "Se aplica al elegirlo y sigue tras reiniciar",
        "Applies as soon as you pick it and survives a reboot",
        "S’applique dès la sélection et persiste après un redémarrage",
        "选择后立即应用，重启后仍保持",
        "Применяется сразу при выборе и сохраняется после перезагрузки",
    ),
    ("Ninguno", "None", "Aucun", "无", "Нет"),
    ("Respiración", "Breathing", "Respiration", "呼吸", "Дыхание"),
    ("Arcoíris", "Rainbow", "Arc-en-ciel", "彩虹", "Радуга"),
    ("Onda", "Wave", "Onde", "波浪", "Волна"),
    ("Cometa", "Comet", "Comète", "彗星", "Комета"),
    ("Fuego", "Fire", "Feu", "火焰", "Огонь"),
    ("Aurora", "Aurora", "Aurore", "极光", "Сияние"),
    ("Velocidad", "Speed", "Vitesse", "速度", "Скорость"),
    (
        "Velocidad del efecto en por ciento",
        "Effect speed in percent",
        "Vitesse de l’effet en pourcentage",
        "效果速度百分比",
        "Скорость эффекта в процентах",
    ),
    ("Color del efecto", "Effect colour", "Couleur de l’effet", "效果颜色", "Цвет эффекта"),
    (
        "Lo usan Respiración, Onda y Cometa",
        "Used by Breathing, Wave and Comet",
        "Utilisée par Respiration, Onde et Comète",
        "呼吸、波浪和彗星会使用它",
        "Используется эффектами «Дыхание», «Волна» и «Комета»",
    ),
    (
        "Color base del efecto animado",
        "Base colour of the animated effect",
        "Couleur de base de l’effet animé",
        "动态效果的基础颜色",
        "Базовый цвет анимированного эффекта",
    ),
    (
        "Efecto detenido; vuelve la iluminación estática.",
        "Effect stopped; static lighting is back.",
        "Effet arrêté ; l’éclairage statique est de retour.",
        "效果已停止，恢复静态灯光。",
        "Эффект остановлен; вернулась статическая подсветка.",
    ),
    (
        "Efecto {name} en marcha.",
        "Effect {name} is running.",
        "L’effet {name} est en cours.",
        "效果 {name} 正在运行。",
        "Эффект {name} запущен.",
    ),
    ("En marcha", "Running", "En cours", "运行中", "Работает"),
    ("Efecto de teclado", "Keyboard effect", "Effet du clavier", "键盘效果", "Эффект клавиатуры"),
    ("ninguno", "none", "aucun", "无", "нет"),
    ("desconocido", "unknown", "inconnu", "未知", "неизвестно"),
    (
        "{kind} al {speed} %",
        "{kind} at {speed} %",
        "{kind} à {speed} %",
    ...
    (
        ...
        "Effect {name} · animated by the local service",
        "Effet {name} · animé par le service local",
        "效果 {name} · 由本地服务播放",
        "Эффект {name} · анимируется локальной службой",
    ),
