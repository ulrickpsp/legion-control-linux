...
_ENTRIES: Final = (
    ...
    (
        "Ruido, consumo y potencia",
        "Noise, power use and performance",
        "Bruit, consommation et puissance",
        "噪音、功耗和性能",
        "Шум, энергопотребление и мощность",
    ),
    ...
    (
        ...
        "CPU power and fans are applied together in Custom",
        "La puissance CPU et les ventilateurs sont appliqués ensemble en Personnalisé",
        "在自定义模式中，CPU 功耗和风扇会一起应用",
        "В пользовательском режиме мощность CPU и вентиляторы применяются вместе",
    ),
    (
        "Potencia sostenida",
        "Sustained power",
    ...
    (
        "Límite estable de CPU (PPT PL1 / SPL)",
        "Steady CPU limit (PPT PL1 / SPL)",
        "Limite CPU stable (PPT PL1 / SPL)",
        "稳定 CPU 限制（PPT PL1 / SPL）",
        "Постоянный лимит CPU (PPT PL1 / SPL)",
    ),
    ("Potencia lenta", "Slow power", "Puissance lente", "短时功耗", "Кратковременная мощность"),
    (
        "Techo temporal de CPU (PPT PL2 / SPPT)",
        "Temporary CPU ceiling (PPT PL2 / SPPT)",
        "Plafond CPU temporaire (PPT PL2 / SPPT)",
        "临时 CPU 上限（PPT PL2 / SPPT）",
        "Временный потолок CPU (PPT PL2 / SPPT)",
    ),
    ...
    (
        ...
        "Filtered, hysteretic curve · continues after you close the window",
        "Courbe filtrée avec hystérésis · continue après fermeture de la fenêtre",
        "带滤波和滞后的曲线 · 关闭窗口后仍继续运行",
        "Кривая с фильтрацией и гистерезисом · продолжает работать после закрытия окна",
    ),
    (
    ...
    (
        "Cambios guardados por el firmware",
        "Changes saved by firmware",
        "Modifications enregistrées par le firmware",
        "更改由固件保存",
        "Изменения сохранены прошивкой",
    ),
    (
        "Detectando modelo…",
        "Detecting model…",
    ...
    (
        "Borrador local · un solo cambio al pulsar Aplicar",
        "Local draft · one change when Apply is pressed",
        "Brouillon local · une seule modification en cliquant sur Appliquer",
        "本地草稿 · 点击应用后一次性更改",
        "Локальный черновик · одно изменение по нажатию «Применить»",
    ),
    (
    ...
    (
        ...
        "Off by default · works while Legion Control is open",
        "Désactivé par défaut · fonctionne tant que Legion Control est ouvert",
        "默认关闭 · Legion Control 打开时生效",
        "Отключено по умолчанию · работает, пока Legion Control открыт",
    ),
    (
