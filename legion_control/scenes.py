    ...
    def __post_init__(self) -> None:
        ...
        if self.fan_policy.mode is FanMode.AUTO:
            if self.profile == "custom" or self.power_limits is not None:
                raise ValueError(
                    "Una escena automática no puede incluir potencia Custom."
                )
        elif self.profile != "custom" or self.power_limits is None:
            raise ValueError(
                "Una escena con ventilación manual necesita perfil y potencia Custom."
            )
...
def _scenes_from_document(document: dict[str, Any]) -> dict[SceneSlot, Scene]:
    ...
    scenes = {
        ...
        for scene in (_scene_from_document(item),)
    }
    if set(scenes) != set(SceneSlot):
...
def _scene_from_document(document: Any) -> Scene:
    ...
    power_limits = (
        None
        if power_document is None
        else power_limits_from_document(power_document)
    )
    rgb_configuration = rgb_configuration_from_document(
        document["rgb_configuration"]
    )
    return Scene(
        slot=slot,
        profile=profile,
...
def _load_document(payload: str) -> dict[str, Any]:
    ...
    if document["version"] != SCENE_CONFIG_VERSION:
        raise ValueError(
            f"Solo se admiten escenas versión {SCENE_CONFIG_VERSION}."
        )
    return document
